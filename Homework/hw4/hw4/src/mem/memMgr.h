...
#define toSizeT(t)      (t)+(SIZE_T_1)-((t)-1)%SIZE_T  // TODO
//
...
class MemRecycleList
{
   ...
   void reset() {
      // TODO
      if (getNextList()) { getNextList()->reset(); }
      _first = 0;
      _nextList = 0;
   }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
      // TODO
      MemBlock<T> *blk = _activeBlock;
      size_t numBlocks = getNumBlocks();
      for (size_t i = 0; i < numBlocks-1; ++i)
      {
         MemBlock<T> *tmp = blk->getNextBlock();
         delete blk;
         blk = tmp;
      }
      _activeBlock = blk;
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      while (true)
      {
         ...
         {
            if (list->getNextList() != 0) { list = list->getNextList(); }
            else { break; }
         }
      }
      MemRecycleList<T>* newRecycleList = new MemRecycleList<T>(n);
      list->setNextList(newRecycleList);
   ...
   T* getMem(size_t t) {
      ...
      // TODO
      MemRecycleList<T>* memRecycleList = getMemRecycleList(n);
      if (memRecycleList->numElm() != 0) 
      {
         ret = memRecycleList->popFront();
         #ifdef MEM_DEBUG
         cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
         #endif // MEM_DEBUG
         return ret;
